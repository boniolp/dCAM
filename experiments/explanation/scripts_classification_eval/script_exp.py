...
def generate_output_file(dataset_names,model_names):
	...
	for file in os.listdir('../results_classification/log/'):
		if '.txt' in file:
			model_name = file.split('_')[0]
			dataset_name  = file.replace(model_name+'_','').replace('.txt','')
			with open('../results_classification/log/'+file ,"r") as f:
				for line in f:
					val = line.rstrip()
			df_results.at[dataset_name,model_name] = float(val.split('-')[0])

	df_results.to_csv('../results_classification/merged_results_classification.csv')

...
if __name__ == '__main__':
	...
	try:
		with open("submit_all_influence_k.sh",'w') as f:
			f.write("#!/bin/bash \n")
		count = 0
		for name in dataset_names: 
			...
			for model_name,type_input in model_names:
				submit_name = 'auto_script/submit_{}_{}_influence_k.sh'.format(model_name,name.strip('.pickle'))
				
				with open("submit_all_influence_k.sh",'a') as f:
					f.write("sbatch {}\n".format(submit_name))
					#f.write("sleep 5 \n")
				count += 1
				with open(submit_name,'w') as f:
					script_name = 'script_exp_dataset_influence_k.py {} {} {} {}'.format(model_name,type_input,dataset_name, parameters['train_test_r'])
					name_job = 'synth_influence_k_{}_{}'.format(model_name,name.strip('.pickle'))
